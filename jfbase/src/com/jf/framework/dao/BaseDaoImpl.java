...
/**
 * @描述:<p>主要抽取dao操作的公用方法 </p>
 *
 * @作者: 叶平平(yepp)
 *
 * @时间: 2011-11-12 上午10:10:17
 * @param <T>
 */
...
public  class BaseDaoImpl<T> implements BaseDao<T> {
	...
	@Autowired
	@Resource(name="daoJdbcCallBase")
	private DaoJdbcCallBase daoJdbcCallBase;
	...
	public void updateBasicField(T entity){
		this.daoHibernateBase.updateBasicField(entity);
	}
	...
	public void lock(Object entity, LockMode lockMode){
		this.sessionFactory.getCurrentSession().lock(entity, lockMode);
	}
	...
	public List<T> queryForList(String sql){
		 return this.daoJdbcBase.queryForList(sql);	
	}
	/**
	 * @描述:<p>  </p>
	 *
	...
	public List<T> queryForList(String sql, int size){
		return this.daoJdbcBase.queryForList(sql, size);
	}
	...
	public List<T> queryForList(String sql, List list){
		return this.daoJdbcBase.queryForList(sql, list.toArray());
	}
	...
	public List<T> queryForList(String sql, List list, int size){
		return this.daoJdbcBase.queryForList(sql, list.toArray(), size);
	}
	...
	public Map queryForMap(String sql){
		return this.daoJdbcBase.queryForMap(sql);
	}
	...
	public Map queryForMap(String sql, List list){
		return this.daoJdbcBase.queryForMap(sql, list.toArray());
	}
	...
	public T queryForObject(String sql, List list, Class<T> clazz){
		return (T)this.daoJdbcBase.queryForObject(sql, clazz, list.toArray());
	}
	...
	public T findObject(String hql, List params){
		return (T) this.daoHibernateBase.findOneEntity(hql, params);
	}
	/**
	...
	public boolean isPropertyExist(Class<T> clazz, String propertyName,Object propertyValue){
		return this.daoHibernateBase.isPropertyExist(clazz, propertyName, propertyValue);
	}
	/**
	 * @描述:<p>  </p>
	 *
	...
	public List findList(String hql,Object values[]){
		return null;//this.daoHibernateBase..find(hql, values);
	}
	...
	public String getCountSql(String sql){
		return this.daoJdbcBase.getCountSql(sql);
	}
	/**
	 * @描述:<p>HQL分页查询  </p>
	 *
	...
	public PageModel<T> findByHql(String hql , int start ,int end){
		return this.daoHibernateBase.findByHql(hql, start, end);
	...
	public PageModel<T> findlHql(String hql , int start ,int end, List list){
		return this.daoHibernateBase.findByHql(hql, start, end, list);
	}
	/**
	 * @描述:<p>SQL分页查询  </p>
	 *
	...
	public PageModel queryBySql(String sql,int strat, int end){
		return this.daoJdbcBase.queryBySql(sql, strat, end, null);
	}
	...
	public PageModel queryBySql(String sql, int strat,int end, List list){
		return this.daoJdbcBase.queryBySql(sql, strat, end, list.toArray());
	}
