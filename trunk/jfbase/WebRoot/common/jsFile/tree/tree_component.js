...
(function($) {
	...
	//添加去除树事件触发方法；针对页面的局部刷新，造成的多次触发问题（对应事件live方法）；在生成树前调用改方法；live
	$.fn.destroyTreeEvent =function() {
				$(this).die(".jstree");
				$(this).find("li").die("click");
				$(this).find("li a").die(".jstree");
				$(this).find("li a").die("click.jstree").die("dblclick.jstree").die("contextmenu.jstree").die("mouseover.jstree").die("mouseout.jstree").die("mousedown.jstree");
	}
				...
				if(this.settings.languages.length) {
					for(var i = 0; i < this.settings.languages.length; i++) {
						...
						if(data.data[this.settings.languages[i]] && data.data[this.settings.languages[i]].icon && this.settings.theme_name != "themeroller") {
							...
							attr["style"] += " ; background-image:url('" + $.webPath + icn + "'); ";
						}
						str += "<a";
						for(var j in attr) {
